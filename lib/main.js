...
function iterLogspace( start, stop, N, options ) {
	...
	if ( !isNumber( start ) || isnan( start ) ) {
		throw new TypeError( format( '0PJ49,Hq', start ) );
	}
	if ( !isNumber( stop ) || isnan( stop ) ) {
		throw new TypeError( format( '0PJ4A,Hr', stop ) );
	}
		...
		if ( isNonNegativeInteger( N ) ) {
			...
			if ( hasOwnProp( N, 'base' ) ) {
				if ( !isPositiveNumber( N.base ) ) {
					throw new TypeError( format( '0PJ4D,Gu', 'base', N.base ) );
				}
				b = N.base;
			} else {
				b = DEFAULT_BASE;
			...
			throw new TypeError( format( '0PJ4E,Lo', N ) );
		}
	} else if ( arguments.length >= 4 ) {
		if ( !isNonNegativeInteger( N ) ) {
			throw new TypeError( format( '0PJ2m,L9', N ) );
		}
		n = N;
		if ( !isPlainObject( options ) ) {
			throw new TypeError( format( '0PJ43,Lp', options ) );
		}
		if ( hasOwnProp( options, 'base' ) ) {
			if ( !isPositiveNumber( options.base ) ) {
				throw new TypeError( format( '0PJ4D,Gu', 'base', options.base ) );
			}
			b = options.base;
		} else {
			b = DEFAULT_BASE;
