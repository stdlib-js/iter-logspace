...
function iterLogspace( start, stop, N, options ) {
	...
	if ( !isNumber( start ) || isnan( start ) ) {
		throw new TypeError( format( '0PJ49', start ) );
	}
	if ( !isNumber( stop ) || isnan( stop ) ) {
		throw new TypeError( format( '0PJ4A', stop ) );
	}
		...
		if ( isNonNegativeInteger( N ) ) {
			...
			if ( hasOwnProp( N, 'base' ) ) {
				if ( !isPositiveNumber( N.base ) ) {
					throw new TypeError( format( '0PJ4D', 'base', N.base ) );
				}
				b = N.base;
			} else {
				b = DEFAULT_BASE;
			...
			throw new TypeError( format( '0PJ4E', N ) );
		}
	} else if ( arguments.length >= 4 ) {
		if ( !isNonNegativeInteger( N ) ) {
			throw new TypeError( format( '0PJ2m', N ) );
		}
		n = N;
		if ( !isPlainObject( options ) ) {
			throw new TypeError( format( '0PJ43', options ) );
		}
		if ( hasOwnProp( options, 'base' ) ) {
			if ( !isPositiveNumber( options.base ) ) {
				throw new TypeError( format( '0PJ4D', 'base', options.base ) );
			}
			b = options.base;
		} else {
			b = DEFAULT_BASE;
